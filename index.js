 ...
 * @param headers {Object} optional headers to pass to S3
...
Retry.prototype.upload = function(sourcePath, destination, headers, cb) {
  ...
  fs.readFile(sourcePath, function(err, file) {
    ...
    if (!headers['Content-Type']) {
      headers['Content-Type'] = self.mime.lookup(destination);
    }
 ...
 */
Retry.prototype.uploadFiles = function(files, headers, cb) {
  if (typeof(headers) === 'function') {
    cb = headers;
    headers = this.defaultHeaders;
  }
  ...
  files.forEach(function(file) {
    self.upload(file.src, file.dest, headers, done);
  });
}
